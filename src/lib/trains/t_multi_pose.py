...
# from models.losses import t_RegL1Loss, t_RegLoss, t_RegWeightedL1Loss
from models.decode import multi_pose_decode
from models.utils import _sigmoid, flip_tensor, flip_lr_off, flip_lr
from utils.debugger import Debugger
from utils.post_process import multi_pose_post_process
from utils.oracle_utils import gen_oracle_map
from .t_base_trainer import BaseTrainer
...
class MultiPoseLoss(torch.nn.Module):
    ...
    def forward(self, outputs, t_outputs,batch):
        opt = self.opt
        hm_loss, wh_loss, off_loss = 0, 0, 0
        hp_loss, off_loss, hm_hp_loss, hp_offset_loss = 0, 0, 0, 0
        t_hm_loss, t_wh_loss, t_off_loss = 0, 0, 0
        t_hp_loss, t_off_loss, t_hm_hp_loss, t_hp_offset_loss = 0, 0, 0, 0

        for s in range(opt.num_stacks):
            output = outputs[s]
            t_output = t_outputs[s]

            output['hm'] = _sigmoid(output['hm'])
            t_output['hm'] = _sigmoid(t_output['hm'])

            if opt.hm_hp and not opt.mse_loss:
                output['hm_hp'] = _sigmoid(output['hm_hp'])
                t_output['hm_hp'] = _sigmoid(t_output['hm_hp'])
            ...
            t_hm_loss += self.crit_teacher(output['hm'], t_output['hm'])
            if opt.dense_hp:
                ...
                # t_hp_loss += self.t_crit_kp(output['hps'], batch['hps_mask'],
                #                         batch['ind'], t_output['hps']) / opt.num_stacks

                # t_hp_loss += self.crit_teacher(output['hps'], t_output['hps'])
            if opt.wh_weight > 0:
                wh_loss += self.crit_reg(output['wh'], batch['reg_mask'],
                                         batch['ind'], batch['wh']) / opt.num_stacks
                # t_wh_loss += self.t_crit_reg(output['wh'], batch['reg_mask'],
                #                          batch['ind'], t_output['wh']) / opt.num_stacks
                # t_wh_loss += self.crit_teacher(output['wh'], t_output['wh'])

            if opt.reg_offset and opt.off_weight > 0:
                off_loss += self.crit_reg(output['reg'], batch['reg_mask'],
                                          batch['ind'], batch['reg']) / opt.num_stacks
                # t_off_loss += self.t_crit_reg(output['reg'], batch['reg_mask'],
                #                           batch['ind'], t_output['reg']) / opt.num_stacks
                # t_off_loss += self.crit_teacher(output['reg'], t_output['reg'])

            if opt.reg_hp_offset and opt.off_weight > 0:
                hp_offset_loss += self.crit_reg(
                    output['hp_offset'], batch['hp_mask'],
                    batch['hp_ind'], batch['hp_offset']) / opt.num_stacks
                # t_hp_offset_loss += self.t_crit_reg(
                #     output['hp_offset'], batch['hp_mask'],
                #     batch['hp_ind'], t_output['hp_offset']) / opt.num_stacks

                # t_hp_offset_loss += self.crit_teacher(output['hp_offset'], t_output['hp_offset'])
            if opt.hm_hp and opt.hm_hp_weight > 0:
                hm_hp_loss += self.crit_hm_hp(
                    output['hm_hp'], batch['hm_hp']) / opt.num_stacks
                # t_hm_hp_loss += self.crit_hm_hp(
                #     output['hm_hp'], t_output['hm_hp']) / opt.num_stacks
                t_hm_hp_loss += self.crit_teacher(output['hm'], t_output['hm'])/ opt.num_stacks
        ...
        t_loss = opt.hm_weight * t_hm_loss + opt.wh_weight * t_wh_loss + \
               opt.off_weight * t_off_loss + opt.hp_weight * t_hp_loss + \
               opt.hm_hp_weight * t_hm_hp_loss + opt.off_weight * t_hp_offset_loss


        # all_loss = loss+100*t_loss
        all_loss = t_loss

        loss_stats = {'loss': loss, 'hm_loss': hm_loss, 'hp_loss': hp_loss,
                      'hm_hp_loss': hm_hp_loss, 'hp_offset_loss': hp_offset_loss,
                      'wh_loss': wh_loss, 'off_loss': off_loss, 'all_loss':all_loss,'t_loss':t_loss}
        return all_loss, loss_stats


class t_MultiPoseTrainer(BaseTrainer):
    def __init__(self, opt, model,t_model, optimizer=None):
        super(t_MultiPoseTrainer, self).__init__(opt, model,t_model ,optimizer=optimizer)

    def _get_losses(self, opt):
        loss_states = ['loss', 'hm_loss', 'hp_loss', 'hm_hp_loss',
                       'hp_offset_loss', 'wh_loss', 'off_loss','all_loss','t_loss']
